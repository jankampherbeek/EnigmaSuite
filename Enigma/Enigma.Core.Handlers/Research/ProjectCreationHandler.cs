...
public sealed class ProjectCreationHandler : IProjectCreationHandler
{
    ...
    public bool CreateProject(ResearchProject project, out int errorCode)
    {
        ...
        {
            errorCode = ErrorCodes.RESEARCH_PROJFOLDER_EXISTS;
            return false;
        }
        if (!CreateFolder(project.Name))
        {
            errorCode = ErrorCodes.RESEARCH_CANNOT_CREATE_PROJFOLDER;
            return false;
        }
        if (!CreateFolder(project.Name + @"\results"))
        {
            errorCode = ErrorCodes.RESEARCH_CANNOT_CREATE_RESULTSFOLDER;
            return false;
        }
        if (!ParseJson(project, out string jsonText))
        {
            errorCode = ErrorCodes.RESEARCH_CANNOT_PARSE_PROJECT2_JSON;
            return false;
        }
        if (!WriteJsonToFile(jsonText, project))
        {
            errorCode = ErrorCodes.RESEARCH_CANNOT_WRITE_JSON4_PROJECT;
            return false;
        }
        if (!CopyDataFile(project))
        {
            errorCode = ErrorCodes.RESEARCH_CANNOT_COPY_DATAFILE;
            return false;
        }
