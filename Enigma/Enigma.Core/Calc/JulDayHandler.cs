...
/// <inheritdoc/>
public sealed class JulDayHandler(IJulDayCalc julDayCalc, IJulDayFacade julDayFacade) : IJulDayHandler
{
    /// <inheritdoc/>
    public JulianDayResponse CalcJulDay(SimpleDateTime dateTime)
    {
        ...
        try
        {
            julDayUt = julDayCalc.CalcJulDayUt(dateTime);
            deltaT = julDayCalc.CalcDeltaT(julDayUt);
    ...
    public SimpleDateTime CalcDateTime(double jdNr, Calendars cal)
    {
        return julDayFacade.DateTimeFromJd(jdNr, cal);
    }
}