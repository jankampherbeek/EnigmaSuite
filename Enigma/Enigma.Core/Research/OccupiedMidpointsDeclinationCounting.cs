    ...
    : IOccupiedMidpointsDeclinationCounting
{
    ...
    private CountOfOccupiedMidpointsDeclResponse PerformCount(IEnumerable<CalculatedResearchChart> charts,
        CountOccupiedMidpointsDeclinationRequest request)
    {
        ...
        foreach (var mpStructure in from calcResearchChart in charts
                 ...
                 select pointsMapping.MapFullPointPos2PositionedPoint(relevantChartPointPositions,
                     CoordinateSystems.Equatorial, false)
                 into posPoints
                 select midpointsHandler.RetrieveOccupiedMidpointsInDeclination(posPoints, orb)
        ...
        return new CountOfOccupiedMidpointsDeclResponse(request, ctrlGroupFactor, allCounts);
    }
