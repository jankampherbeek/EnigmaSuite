    ...
    : IOccupiedMidpointsCounting
{
    ...
    private CountOfOccupiedMidpointsResponse PerformCount(IEnumerable<CalculatedResearchChart> charts, CountOccupiedMidpointsRequest request)
    {
        ...
        foreach (var mpStructure in from calcResearchChart in charts 
                 ...
                 select pointsMapping.MapFullPointPos2PositionedPoint(relevantChartPointPositions, CoordinateSystems.Ecliptical, true) 
                 into posPoints select midpointsHandler.RetrieveOccupiedMidpoints(posPoints, dialSize, orb) 
